...
type Props = {
  ...
  positions: SharedValue<{[key: string]: TrackType['id']}>;
...
export const MovableTrack: FC<Props> = ({
  ...
  const top = useSharedValue((positions.value[id] || index) * TRACK_HEIGHT);
  ...
  useAnimatedReaction(
    ...
    (currentValue, previousValue) => {
      if (currentValue !== previousValue) {
        if (!moving) {
          top.value = withSpring(currentValue * TRACK_HEIGHT);
        }
      }
    },
    [moving],
  ...
  const gestureHandler = useAnimatedGestureHandler({
    ...
    onFinish() {
      top.value = positions.value[id] * TRACK_HEIGHT;
  ...
  const animatedStyles = useAnimatedStyle(
    () => ({
      backgroundColor: runOnJS(getColors)(theme, Colors.background)!,
      left: 0,
      right: 0,
      position: 'absolute',
      top: top.value,
